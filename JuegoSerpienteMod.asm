...
.data
    dimX DB ?   ;dimensionX
    dimY DB ?   ;dimensionX
    mensajeBienvenida db 'Bienvenido al juego de la SERPIENTE$'
...
mostrar_bienvenida proc
    ...
    mov ah, 09h     ; Función para mostrar cadena terminada en $
    int 21h
    ret
mostrar_bienvenida endp

asignar_dimensiones proc
    ; Asignar valores fijos
    mov al, 26h         ; AL = 38 decimal
    mov ccX, al         ; ccX = 38
    add ccX, 1          ; ccX = 39 (para centrar cursor)
    add al, al          ; al = 2 * dimX
    add al, 1           ; al = 2 * dimX + 1
    mov dimX, al        ; dimX = 2*valor + 1
    ...
    add ccY, 1          ; ccY = 10
    add al, al          ; al = 2 * dimY
    add al, 1           ; al = 2 * dimY + 1
    mov dimY, al        ; dimY = 2*valor + 1
...
imprimir_fruta proc
    call movFruta           ; mueve el cursor a la posición de la fruta

    mov ah, 09h             ; función para imprimir carácter con color
    mov al, fruta           ; carácter de la fruta
    mov bh, 0               ; página de video
    mov bl, 0Ch             ; atributo de color (rojo brillante = 0Ch)
    mov cx, 1               ; número de veces que se imprimirá
    int 10h
...
ciclo2:
    ...
    MOV DL , posLis[SI]  ; Columna (X)
    INC SI
    MOV DH , posLis[SI]  ; Fila (Y)
    INC SI

    ; Posicionar el cursor con INT 10h (Función AH=02h)
    MOV BH , 0           ; Página de video (0)
    MOV AH , 02h         ; Función de mover el cursor
    INT 10h

    ; Imprimir el carácter con color verde usando INT 10h (Función AH=09h)
    MOV DL , cuerpo
    MOV AH , 02h
    INT 21h 
